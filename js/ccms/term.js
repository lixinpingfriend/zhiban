...
function Term(id,name,contentDiv){
	...
	this.printHTML = function(){//全部输出出来
		var str = "";
		var beginIndex = 0;
		for(var i=0;i<this.items.length;i++){
...
function TermItem(id,name,code,is_matrix,show_type,is_page_break,show_order,remark,is_matrix_transpose,is_list_mline,type_id){
	...
	this.toHTML = function(){
		...
		var str = '<div id="'+this.code+'" name="'+this.code+'" class="panel panel-default" style="display:none;">';
		
		str += '<div class="panel-heading"><h1 class="panel-title">'+this.name+(this.remark!=""?("&nbsp;&nbsp;&nbsp;【备注】"+this.remark):"")+'</h1></div>';
...
//保存题目选择项对象
function TermList(id,item_id,list_name,list_code,list_score,show_order,show_type,is_unspeak,remark,parent_type,item_code){
	this.id = id;
	...
	this.toHTML = function(){//非矩阵题目时的选择项输出成HTML格式
		...
		switch(this.parent_type){
			case "0"://文本输入
				str = this.list_name+"<input type='text' name='"+this.item_code+"' size='20' id='"+this.item_code+"_"+this.list_code+"'>";
			...
			case "1"://多选一
				switch(this.show_type){
					case "0"://标签
						...
						break;
					case "1"://文本
						str = "<input type='text' name='"+this.item_code+"_text' size='20' id='"+this.item_code+"_"+this.list_code+"_text'>";
						if(this.remark!="" && this.remark!=null){
							str = str + "【"+this.remark+"】";
						}
						break;
					case "2"://标签+文本
						str = "<input type='radio' name='"+this.item_code+"' value='"+this.list_score+"' id='"+this.item_code+"_"+this.list_code+"'>"
						+"<label for='"+this.item_code+"_"+this.list_code+"'>"+this.list_name+"</label>"
						+"&nbsp;&nbsp;<input type='text' name='"+this.item_code+"_text' size='20' id='"+this.item_code+"_"+this.list_code+"_text'>";
						if(this.remark!="" && this.remark!=null){
							str = str + "【"+this.remark+"】";
						}
						break;
					case "3"://标签+下拉框
						...
						if(this.droplist.length > 0){
							str += "&nbsp;&nbsp;<select name='"+this.item_code+"_"+this.list_code+"_select' id='"+this.item_code+"_"+this.list_code+"_select'>";
	...
	this.toMatrixHTML = function(item_code,show_type){//矩阵题目时的选择项输出成HTML格式
		...
		switch(show_type){
			case "0"://文本输入
				str = "<input type='text' name='"+item_code+"' size='20' id='"+item_code+"_"+this.list_code+"'>";
				break;
			case "1"://多选一
				switch(this.show_type){
					case "0"://标签
					...
					case "1"://文本
						...
						break;
					case "2"://标签+文本
						str = "<input type='radio' name='"+item_code+"' value='"+this.list_score+"' id='"+item_code+"_"+this.list_code+"'>&nbsp;&nbsp;<input type='text' name='"+item_code+"_text' size='20' id='"+item_code+"_"+this.list_code+"_text'>";
						break;
				}
				break;
			case "2"://复选
				switch(this.show_type){
					...
					case "1"://文本
						...
						break;
					case "2"://标签+文本
						str = "<input type='checkbox' name='"+item_code+"' value='"+this.list_score+"' id='"+item_code+"_"+this.list_code+"'>&nbsp;&nbsp;<input type='text' name='"+item_code+"_text' size='20' id='"+item_code+"_"+this.list_code+"_text'>";
						break;
				}
				break;
		}
