...
if("${fld:wait_time}" != ""){
	...
	if(w > 59){//hour
		var m = w%60;
		var h = (w-m)/60;
		setComboValue("hour_v",h,"formEditor");
		w = m;
	}
	setComboValue("min_v",w,"formEditor");
}

setCheckboxValue("node_type","${fld:node_type@js}","formEditor");
setCheckboxValue("ob_type","${fld:ob_type@js}","formEditor");

setCheckboxValue("is_auto_assign","${fld:is_auto_assign@js}","formEditor");
document.formEditor.limit_dial_count.value = "${fld:limit_dial_count}";

setCheckboxValue("grab_flag","${fld:grab_flag@js}","formEditor");
setCheckboxValue("grab_skip_flag","${fld:grab_skip_flag@js}","formEditor");
setCheckboxValue("grab_flag_scope","${fld:grab_flag_scope}","formEditor");
setComboValue("email_send_type","${fld:email_send_type}","formEditor");
