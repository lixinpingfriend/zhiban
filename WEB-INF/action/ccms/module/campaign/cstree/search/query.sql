...
select
	...
	'' as super,
	'' as addon,
cast((case when t.job_priority is null then 0 else t.job_priority end + 70) as varchar(4000)) as show_order
from 
	cs_job t
...
select
	...
	'' as addon,
cast((case when t.job_priority is null then 0 else t.job_priority end + 80) as varchar(4000)) as show_order
from 
	cs_job t
	inner join cs_campaign c
	on t.campaign_id = c.tuid
...
select
	...
	, '&nbsp;' , 
case when t.phrase_name is null then '' else t.phrase_name end),0,50) as show_name,
	case when t.parent_id is null then 
...
from 
	...
	on t.form_id = f.tuid
	left join t_field d
	on (f.table_id=d.table_id and t.clause_code=d.field_code)

where
	t.is_node = '1'
and
	(t.filter_type='0' or t.filter_type='3')
...
where
	t.is_node = '1'
and
	(t.filter_type='1' or t.filter_type='2')
