...
and(
  ...
  (
  ${fld:validate_date}='1'
  and start_date::date <= {d '${def:date}'} 
  and end_date::date - 1 >= {d '${def:date}'}
  )
  )    
  ${filter}
...
inner join  cs_campaign c on  c.tuid= ta.campaign_id  and c.campaign_type = '0' AND c.campaign_status = '1' and case when ${fld:campaign_name} is not null then c.campaign_name else '1' end like  case when ${fld:campaign_name} is not null then concat('%',${fld:campaign_name},'%') else '1' end 
order by show_order::int desc,ta.scope_name
